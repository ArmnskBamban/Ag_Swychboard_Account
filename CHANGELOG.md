...
### Added
- **Collect Accounts from Email List** command. Select a `.txt` or `.csv` file with one Google email per line, complete the official Google OAuth flow in an incognito/private browser window, and store each account directly in Switchboard.
- **Bulk token import** command for `refresh_token` and `email|refresh_token` files.
- **Ready-to-use VSIX packaging** for GitHub Releases.

### Changed
- Package metadata now points to this fork's GitHub repository.
- Webview build uses a local TypeScript bundler script for reliable Windows packaging.
