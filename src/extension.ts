...
export async function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        ...
        vscode.commands.registerCommand('ag.importAccountsByTokenFile', async () => {
            const result = await accountManager.importAccountsByTokenFile();
            if (result && (result.added > 0 || result.updated > 0)) {
                quotaManager.refresh();
            }
        }),

        vscode.commands.registerCommand('ag.collectAccountsFromEmailFile', async () => {
            const result = await accountManager.collectAccountsFromEmailFile();
            if (result && (result.added > 0 || result.updated > 0)) {
                quotaManager.refresh();
            }
        }),
